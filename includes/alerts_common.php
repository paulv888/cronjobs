...
function replacePlaceholder(&$mess_subject, $params){
	...
	if (preg_match("/\{.*\}/", $mess_subject, $matches)) {
		replaceFields($mess_subject, $mess_text, $params);
	}
}
...
function replaceText(&$params){
	...
	$mess_subject = $params['mess_subject'];
	$mess_text = $params['mess_text'];
...
if (DEBUG_ALERT) {
	echo "<pre>Param Values "; echo "DATA2:"; print_r ($params); echo "</pre>";
	echo "<pre>Param Values "; echo "PATTERN2:"; print_r ($pattern); echo "</pre>";
}
	...
	if (isset($mysql)) {
		if ($props = FetchRows($mysqlp)) {
			...
			if (DEBUG_ALERT) {
				echo "<pre>Device Properties "; echo "DATA3:"; print_r ($newprops); echo "</pre>";
				echo "<pre>Device Properties "; echo "PATTERN3:"; print_r ($pattern); echo "</pre>";
			}
