...
// https://api.weather.gov/gridpoints/BMX/60,77/stations 
	...
	$feedback['commandstr'] = 'https://api.weather.gov/stations/'.$params['commandvalue'].'/observations/latest?require_qc=true';
	
	echo $feedback['commandstr'];
	$get = RestClient::get($feedback['commandstr'],null,setAuthentication($params['device']), "application/json", $params['device']['connection']['timeout']);
	echo "<PRE>";
	print_r($get->getresponse());
	echo "</PRE>";
	exit;
	// debug($get->getresponse(), '$get->getresponse()');
	$feedback['result'] = $get->getresponse();
	return $feedback;
	exit;
