	...
	while(!$abort and !$finished) {
		if($FSM==0) {
			...
			if($plm_cmd_id == '0250') {
				...
				$flag_ext = hexdec(substr($plm_string,16,1))&0b0001;
...
#XXXX				$plm_message .= sprintf("%20s: ",'Link Data').substr($plm_string,14,6)."\n";
			...
			if($plm_cmd_id == '0260') {
				...
				$flag_ext = hexdec(substr($plm_string,10,1))&0b0001;
...
function insteon_message_flags_decode($flags_string) {
	...
	$flag_ext = hexdec(substr($flags_string,0,1))&0b0001;
	...
	$flags_message .= sprintf("%28s: %d\n",'Max Hops',hexdec(substr($flags_string,1,1))&0b0011);
...
*/
	...
	$extended = hexdec(substr($command_string,0,1))&0b0001;
	$msg_type = (hexdec(substr($command_string,0,1))&0b1110)>>1;
