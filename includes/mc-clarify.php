...
function lastXDays($qname) {
		...
		$mysql = 'UPDATE `mc`.`mc_cases_full` SET `sla_typeID` = 5 ';
		if (!$result = mysql_query($mysql)){
						mySqlError($mysql);
						return false;
		}
		...
		$mysql = 'UPDATE `mc`.`mc_cases_full` c JOIN `mc_mi_sla_type_map` m ON `c`.`Case_Type` = `m`.`case_type` SET `c`.`sla_typeID` = `m`.`sla_type` WHERE `m`.`severity` is null';
		if (!$result = mysql_query($mysql)){
						mySqlError($mysql);
						return false;
		}
		...
		$mysql = 'UPDATE `mc`.`mc_cases_full` c JOIN `mc_mi_sla_type_map` m ON `c`.`severity`=`m`.`severity` AND `c`.`Case_Type` = `m`.`case_type` SET `c`.`sla_typeID` = `m`.`sla_type`';
		if (!$result = mysql_query($mysql)){
						mySqlError($mysql);
						return false;
		}
