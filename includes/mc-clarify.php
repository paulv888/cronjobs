...
function clarify($init = "CHANGES") {
	...
	if ($init == "INIT") {
		...
		$result = getCases("Last8Days");							// No Distinct - Last 3 days
		$text = date("Y-m-d H:i:s").": ".$result." Cases Read".CRLF;
		if ($result != "ERROR:") {
			$text = date("Y-m-d H:i:s").": ".$text." Cases/CR's Read".CRLF;			// No Distinct - Last 3 days
