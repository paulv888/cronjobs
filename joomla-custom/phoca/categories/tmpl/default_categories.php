...
foreach ($this->categories as $ck => $cv){
	...
	// print_r($cv);
	// var_dump($cv->title);
	// var_dump($this->tmpl['char_cat_length_name']);
	// echo "</pre>";
	
	//echo '<div class="pg-csv-name"><a href="'.$cv->link.'">'.PhocaGalleryText::wordDelete($cv->title_self, $this->tmpl['char_cat_length_name'], '...').'</a>';
	echo '<div class="pg-csv-name"><a href="'.$cv->link.'">'.PhocaGalleryText::wordDelete($cv->title, $this->tmpl['char_cat_length_name'], '...').'</a>';
	if ($cv->numlinks > 0) {
