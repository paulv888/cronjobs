...
class SocketTransport
{
	...
	public function windoes_readAll()
	{
		...
		// If 1 packet received then return
		//
		//	Need to handle 
		//			2 Messages in 1
		//			1/2 Message bc off timeout?
		//			
		$d = "";
		$r = 0;
		$readTimeout = socket_get_option($this->socket,SOL_SOCKET,SO_RCVTIMEO);
		$buf = '';
		$r += socket_recv($this->socket,$buf,1000,MSG_WAITALL);
		if ($r === false) throw new SocketTransportException('Could not read from socket; '.socket_strerror(socket_last_error()), socket_last_error());
		if ($r === 0 ) {
